...
using OnRecvExactlyHandler = std::function<void(NetErrorCode)>;
...
struct EventLoop : public std::enable_shared_from_this<EventLoop>
{
    ...
    bool initialize();
    bool runOnce(bool isImeediately = false);
...
inline bool EventLoop::runOnce(bool isImeediately/* = false*/)
{ return zn_run(S, isImeediately ? ZN_RUN_CHECK : ZN_RUN_ONCE) > 0; }
